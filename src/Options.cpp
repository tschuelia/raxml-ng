...
std::ostream& operator<<(std::ostream& stream, const Options& opts)
{
      ...
      opts.command == Command::parse || opts.command == Command::ancestral)
  {
    ...
    stream << "  lh-epsilon during slow spr rounds: " << opts.lh_epsilon_slow << endl;
    stream << "  lh-epsilon for full brlen opt: " << opts.lh_epsilon_brlen_full << endl;
    stream << "  lh-epsilon for triplet brlen opt: " << opts.lh_epsilon_brlen_triplet << endl;
