...
void CommandLineParser::parse_options(int argc, char** argv, Options &opts)
{
  ...
  while ((c = getopt_long_only(argc, argv, "", long_options, &option_index)) == 0)
  {
    ...
    switch (option_index)
    {
      ...
      case 59: /* lheps for fast spr rounds */
        if(sscanf(optarg, "%lf", &opts.lh_epsilon_fast) != 1 || opts.lh_epsilon_fast <= 0)
        {
            throw InvalidOptionValueException("Invalid lh_eps for fast spr rounds: " +
                                              string(optarg) +
                                              ", please provide a positive real number.");
        }
        break;
